...
async def saveanswers(request: Request, response: Response, kluc: StringPath, predmet: StringForm, trieda: StringForm, kapitola: StringForm, skupina: StringForm = '', subory: FileListOptional = None):
   proc = request.app.state.proc
   if not check_time(proc, predmet, trieda, skupina, kapitola, kluc):
   ...
   if subory is not None:
      for subor in subory:
         try:
            nazov = Path(subor.filename).name
            obsah = await subor.read()
            with open(f'{adresar}/{predmet}_{trieda}{skupina}_{kapitola}_{kluc}-{nazov}_subor', 'wb') as f:
               f.write(obsah)
   ...
   async with request.form() as form:
      form_data = {k: v for k, v in form.items() if k not in {'predmet', 'trieda', 'skupina', 'kapitola', 'subory'}}
   lock = FileLock(f'{cesta}.lock')
   try:
      await run_in_threadpool(write_answers, lock, cesta, form_data, adresar, predmet, trieda, skupina, kapitola, kluc)
   except Exception as e:
...
def write_answers(lock, cesta, form_data, adresar, predmet, trieda, skupina, kapitola, kluc):
   with lock:
      ...
      try:
         ...
         tree = ET.parse(f'{adresar}/{predmet}_{trieda}{skupina}_{kapitola}.xml', xmlParser)
         root = tree.getroot()
         dat = datetime.now().isoformat(timespec='seconds')
         existujuci = next(iter(root.xpath('test[@id=$id]', id=kluc)), None)
         ...
         tree.write(f'{adresar}/{predmet}_{trieda}{skupina}_{kapitola}.xml', encoding='utf-8', xml_declaration=True, pretty_print=True)
      except Exception as e:
