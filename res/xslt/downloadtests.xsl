...
<xsl:param name="qrdir"/>

<xsl:template match="testy">
   <fo:root xml:lang="sk">
      <fo:layout-master-set>
         <fo:simple-page-master master-name="nie-prazdna-neparna" page-height="29.7cm" page-width="21cm" margin-top="0.5cm" margin-bottom="0.5cm" margin-left="1.5cm" margin-right="1.5cm">
            <fo:region-body margin-top="1.5cm" margin-bottom="2cm"/>
            <fo:region-before precedence="true" extent="1.0cm"/>
            <fo:region-after region-name="nie-prazdna-neparna-footer" precedence="true" extent="1.5cm"/>
         </fo:simple-page-master>
         <fo:simple-page-master master-name="nie-prazdna-parna" page-height="29.7cm" page-width="21cm" margin-top="0.5cm" margin-bottom="0.5cm" margin-left="1.5cm" margin-right="1.5cm">
            <fo:region-body margin-top="1.5cm" margin-bottom="2cm"/>
            <fo:region-before precedence="true" extent="1.0cm"/>
            <fo:region-after region-name="nie-prazdna-parna-footer" precedence="true" extent="1.5cm"/>
         </fo:simple-page-master>
         <fo:simple-page-master master-name="nie-prazdna-posledna" page-height="29.7cm" page-width="21cm" margin-top="0.5cm" margin-bottom="0.5cm" margin-left="1.5cm" margin-right="1.5cm">
            <fo:region-body margin-top="1.5cm" margin-bottom="1.5cm"/>
            <fo:region-before precedence="true" extent="1.0cm"/>
            <fo:region-after region-name="nie-prazdna-posledna-footer" precedence="true" extent="1cm"/>
         </fo:simple-page-master>
         ...
         <fo:page-sequence-master master-name="strany">
            <fo:repeatable-page-master-alternatives>
               <fo:conditional-page-master-reference master-reference="nie-prazdna-posledna" page-position="last" blank-or-not-blank="not-blank"/>
               <fo:conditional-page-master-reference master-reference="nie-prazdna-neparna" odd-or-even="odd" blank-or-not-blank="not-blank"/>
               <fo:conditional-page-master-reference master-reference="nie-prazdna-parna" odd-or-even="even" blank-or-not-blank="not-blank"/>
               <fo:conditional-page-master-reference master-reference="prazdna" blank-or-not-blank="blank"/>
            </fo:repeatable-page-master-alternatives>
               ...
               <fo:table-body>
                  <fo:table-row>
                     <fo:table-cell display-align="center">
                        <fo:block>
                           <fo:external-graphic src="url('{$qrdir}/{@id}.svg')" content-height="1.2cm" content-width="1.2cm" scaling="uniform"/>
                        </fo:block>
                     </fo:table-cell>
                     <fo:table-cell display-align="before">
                        <fo:block font-size="8pt" text-align="center">STREDNÁ PRIEMYSELNÁ ŠKOLA JOZEFA MURGAŠA</fo:block>
                        <fo:block font-size="6pt" text-align="center">Hodnotenie: <xsl:value-of select="$max1"/>%-<xsl:value-of select="$min1"/>%:1&#xA0;&#xA0;&#xA0;<xsl:value-of select="$max2"/>%-<xsl:value-of select="$min2"/>%:2&#xA0;&#xA0;&#xA0;<xsl:value-of select="$max3"/>%-<xsl:value-of select="$min3"/>%:3&#xA0;&#xA0;&#xA0;<xsl:value-of select="$max4"/>%-<xsl:value-of select="$min4"/>%:4&#xA0;&#xA0;&#xA0;<xsl:value-of select="$max5"/>%-<xsl:value-of select="$min5"/>%:5</fo:block>
                     </fo:table-cell>
                     <fo:table-cell display-align="before">
...
<xsl:template match="otazka">
   ...
   <xsl:apply-templates select="miesto" mode="pdf"/>
   <xsl:if test="not(odpoved) and not(miesto)">
...
<xsl:template match="odpoved">
   <fo:table-cell padding-left="0.2cm"> <!-- padding kvoli druhemu stlpcu -->
      <fo:block start-indent="0.55cm" text-indent="-0.55cm">
         <fo:inline font-weight="bold"><xsl:number count="odpoved" format="a) "/></fo:inline>
         <xsl:apply-templates/>
      </fo:block>
   </fo:table-cell>
</xsl:template>
