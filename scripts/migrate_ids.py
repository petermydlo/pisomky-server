# -*- coding: utf-8 -*-
# Migracny skript: prevedie stare otazka/@id (kapitola.kategoria.otazka)
# na nove hash id v tests a answers XML suboroch.
...
TESTS_DIR     = './res/xml/tests'
ANSWERS_DIR   = './res/xml/answers'
...
def build_mapping(predmet):
   """Vytvori slovnik {stare_id: nove_hash_id} pre dany predmet."""
   mapping = {}
      ...
      try:
         ...
         for kat in root.findall('kategoria'):
            for otazka in kat.findall('otazka'):
               ...
               if not hash_id:
                  continue
               # vypocitame stare id pre vsetky kombinacie flags
...
def migrate_xml(cesta, mapping, dry_run=False):
   """Nahradi stare id v XML subore novymi hash id."""
   try:
      xmlParser = ET.XMLParser(remove_blank_text=True)
      tree = ET.parse(cesta, xmlParser)
      zmenene = 0
      nenajdene = []
      for otazka in tree.findall('.//otazka[@id]'):
         stare = otazka.get('id')
         if stare in mapping:
            otazka.set('id', mapping[stare])
            zmenene += 1
         elif '.' in stare:
            # ma bodky ale nenajdene v mapping — upozornenie
            nenajdene.append(stare)
      if nenajdene:
         print(f'  NENAJDENE id v {cesta}: {nenajdene}')
...
def main(dry_run=False):
   ...
   for predmet in sorted(predmety):
      ...
      celkom = 0
      for adresar in (TESTS_DIR, ANSWERS_DIR):
         vzor = f'{adresar}/{predmet}/*.xml'
         for cesta in sorted(glob.iglob(vzor)):
            n = migrate_xml(cesta, mapping, dry_run)
            if n > 0:
